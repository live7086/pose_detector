...
bool TreePoseOnePass(Map<String, int> angles) {
  ...
  // r_knee angle
  const int r_knee_perfect_min = 130;
  const int r_knee_good_min = r_knee_perfect_min-20;
  const int r_knee_perfect_max = 150;
  ...
  if (angles.containsKey('r_knee')&&angles.containsKey('r_hip')) {
    ...
    if (r_knee >= r_knee_perfect_min && r_knee <= r_knee_perfect_max /*&&
        r_hip >= r_hip_perfect_min && r_hip <= r_hip_perfect_max */) {
      return true; // 符合 Tree Pose 的條件
    }else if(r_knee >= r_knee_good_min && r_knee <= r_knee_good_max /*&&
